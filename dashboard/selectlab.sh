...
#
# selectlab.sh
# This routine is used to select a lab to download into an image. 
# Date: 2020-02-03
...
readarray -t labs < ~/db2labs/db2labs.list
test=${labs[0]}
test+=(${labs[1]})
echo ${labs[0]}
echo ${labs[1]}
...
printf ">>> Checking to see what lab you want\n" >> $LOGFILE

REPOSITORY=$( 
        cat ~/db2labs/db2labs.list | \
	zenity --list \
	  --title="Lab Selection" \
	  --text="Select one of the labs from the list below" \
	  --radiolist \
	  --height=400 \
	  --width=600 \
	  --print-column=2 \
          --separator='\n' \
	  --column="Selected" --column="Lab Name" --column="Description" \
	    2> /dev/null \
    	  )
...
if [ $REPOSITORY = none ]
  then
	zenity --info \
	       --width=300 \
               --text="No lab selected. System will not refresh." \
		2> /dev/null
	exit 0
fi 
 exit 0
